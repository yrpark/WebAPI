...
select rnum as rnum, table_row_id as table_row_id, to_clob(']}') as JSON from (
  select distinct 5000 as rnum, 1 as table_row_id from pnc_study_summary_path)
--this if very annoying: sql render remove "dual", so I have to trick by using a real table(pnc_study_summary_path) select 5000 as rnum, 1 as table_row_id, to_clob(']}') as JSON from dual
)
GROUP BY
   table_row_id));

...
import org.ohdsi.webapi.search.FullTextIndex;
import org.ohdsi.webapi.search.IndexStartupTasklet;
...
public class VocabularyService extends AbstractDaoService {
    ...
    public JobExecutionResource loadFullTextIndices() {
        IndexStartupTasklet tasklet = new IndexStartupTasklet(this);
        JobParametersBuilder builder = new JobParametersBuilder();
        builder.addString("jobName", "loading FTI");
        return this.jobTemplate.launchTasklet("loadFTI", "loadFITStep", tasklet, builder.toJobParameters());
    }

    public void processConcepts(RowCallbackHandler handler) {
        SourceInfo sourceInfo = sourceService.getPriorityVocabularySourceInfo();
        Source source = getSourceRepository().findBySourceKey(sourceInfo.sourceKey);
        String tableQualifier = source.getTableQualifier(SourceDaimon.DaimonType.Vocabulary);
        String sql_statement = ResourceHelper.GetResourceAsString("/resources/vocabulary/sql/loadFullTextIndex.sql");
        sql_statement = SqlRender.renderSql(sql_statement, new String[]{"CDM_schema"}, new String[]{tableQualifier});
        sql_statement = SqlTranslate.translateSql(sql_statement, "sql server", source.getSourceDialect());

        getSourceJdbcTemplate(source).query(sql_statement, handler);
    }

    /**
     * @summary Perform a lookup of an array of concept identifiers returning
     * the matching concepts with their detailed properties.
    ...
    /**
     * @summary Perform a lookup of an array of concept identifiers returning
     * the matching concepts with their detailed properties, using the default
     * source.
     * @param identifiers an array of concept identifiers
     * @return collection of concepts
     */
    ...
    /**
     * @summary Lookup source codes in the specified vocabulary using the
     * default source.
     * @param sourcecodes array of source codes
     * @return collection of concepts
     */
    ...
    /**
     * @summary find all concepts mapped to the identifiers provided using the
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public ArrayList<Concept> executeLuceneSearch(@QueryParam("q") String searchQuery) {
        FullTextIndex fti = new FullTextIndex();
        StandardAnalyzer analyzer = new StandardAnalyzer();
        ArrayList<Concept> concepts = new ArrayList<>();
        ComplexPhraseQueryParser qp = new ComplexPhraseQueryParser("concept_name", analyzer);
        searchQuery = QueryParserUtil.escape(searchQuery);
        try {
            Query q = qp.parse(searchQuery);                
            IndexReader reader = DirectoryReader.open(fti.getDirectory());
            IndexSearcher searcher = new IndexSearcher(reader);
            TopDocs docs = searcher.search(q, 50);
            for (ScoreDoc scoreDoc : docs.scoreDocs) {
                Document d = searcher.doc(scoreDoc.doc);                
                Concept c = new Concept();
                c.conceptName = d.get("concept_name");
                c.conceptId = Long.parseLong(d.get("concept_id"));
                c.conceptClassId = d.get("concept_class_id");
                c.conceptCode = d.get("concept_code");
                c.domainId = d.get("domain_id");
                c.invalidReason = d.get("invalid_reason");
                c.standardConcept = d.get("standard_concept");
                c.vocabularyId = d.get("vocabulary_id");
                concepts.add(c);
            }
